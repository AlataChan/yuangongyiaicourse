# Designing Agent Skills at Perplexity（延伸阅读 · 英文原文）

> 课程导读见同目录 [`skill-standards.md`](../skill-standards.md) / [`skill-standards.html`](../skill-standards.html)。  
> 来源：Perplexity Agents 团队工程实践文章（课程归档副本）。

---

Title: Designing, Refining, and Maintaining Agent Skills at Perplexity

URL Source: https://research.perplexity.ai/articles/designing-refining-and-maintaining-agent-skills-at-perplexity
...
![Image 1](https://framerusercontent.com/images/StL77jSrvpQypM4nJ5omvZdkHQ.png)
...
This is the hardest line in the Skill. It’s a routing trigger, not documentation. To get the name and the description right, you don't care about the content of the Skill. You only care about whether the Skill is loaded and injected at the right points and is free of off-target side effects, which is the number one failure mode. Every time you add an additional Skill, you risk making every _other_ Skill slightly worse, so you need to make sure that you're minimizing regression.
...
Heavy docs loaded only when a condition is met"Read `api-errors.md` if API returns non-200"
`assets/`

Output templates the agent copies and fills`report-template.md`, output schemas
`config.json`
