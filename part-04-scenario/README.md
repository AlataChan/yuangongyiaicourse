...
本课贯穿虚构机构「青澜环境政策研究中心」。第 3 课时，青澜已经有一个项目助理，可以回答知识库问题，也能调用活动复盘技能。但当同事要求「帮我把这次社区减塑活动做成公众号推文，今晚发」时，单一助理容易跳过资料核对和合规审核。

第 4 课把它拆成 5 个角色：

| 角色 | 负责什么 | 输入 | 输出 | 人工介入点 |
|------|----------|------|------|------------|
| 主控智能体 | 判断任务类型、分派下一步 | 用户任务、项目助理边界卡 | 任务分派 + 交接包 | 用户意图不清时追问 |
| 资料员智能体 | 搜集活动事实与来源 | 活动记录、知识库、公开资料 | 素材卡 + 缺失项 | 找不到关键事实时停止 |
| 结构员智能体 | 把素材排成文章结构 | 素材卡、历史推文 | 提纲 + 语气基准 | 选题角度需人工确认 |
| 写作员智能体 | 写公众号初稿 | 提纲、素材卡、语气基准 | 初稿 + 配图建议 | 不补没有来源的事实 |
| 审核员智能体 | 查隐私、承诺、来源、发布风险 | 初稿、脱敏规则、合规清单 | 发布前问题清单 | 发布前必须人工确认 |

可复用现有第 4 课技能样板：资料员可参考 `env-ngo-wechat-research`，结构员可参考 `env-ngo-wechat-template`，写作员可参考 `env-ngo-wechat-humanize`，审核员可参考 `env-ngo-wechat-publish` 与 `env-ngo-story-desensitize`。
...
| 模块 | 推荐主场景 | 适合拆出的智能体 |
|------|------------|------------------|
| M1 项目文书链 | 资助申请 / 项目书 / 结项报告 | 资料员、文书员、指标核对员、审核员 |
| M2 案例与知识沉淀 | 原始记录 → 脱敏案例 → 入库 | 记录整理员、脱敏员、案例萃取员、入库审核员 |
| M3 传播与运营 | 活动素材 → 公众号草稿 → 发布前检查 | 资料员、结构员、写作员、审核员 |
...
| 4.3 青澜案例拆解 | 20 分钟 | 公众号任务小队角色、交接、审核点 | 案例参考图 |
| 4.4 自己动手拆 | 25 分钟 | 角色卡、交接包、共享状态 | 多智能体协作图 |
| 4.5 协作验收 | 15 分钟 | 5 条测试 + 失败写回 | 修正清单 |
